...
class tick_processor():
    def __init__(self, MT_directory_path, 
        ...
        # subscribe to tick data para EURUSD (para tener el precio de conversión)
        self.dwx.subscribe_symbols(['EURUSD'])

        # Variable para almacenar la última cotización de EURUSD
        self.eurusd_price = None  # cuando recibamos ticks de EURUSD lo actualizamos.

    def on_tick(self, symbol, bid, ask):
        # Aquí comprobamos si el símbolo es EURUSD
        if symbol == "EURUSD":
            # Podemos usar el ask, bid o un promedio. Normalmente para conversión se podría usar el midpoint.
            # Ej: usar el mid-price (promedio entre bid y ask) 
            mid_price = (bid + ask) / 2.0
            self.eurusd_price = mid_price
            # O si prefieres usar el bid o el ask directamente.
            # self.eurusd_price = bid
...
PASSWORD = 'ktxzeqagsklsxolp' #application password
...
def extraer_datos_orden(email_body):
    ...
    if order_info and occurred and signal:
        ...
        if match:
            ...
            # Aquí podrías escribir la orden en CSV si quisieras:
            # escribir_orden_en_archivo(order_data)
            # Pero no es relevante ahora.

            # --- Comienzo del bloque para generar la orden a MQL5 ---
            
            # Elegir el processor según la plataforma seleccionada
            ...
            def calculaMM_DesdeCuentaTS(processor, CapitalTS, ContratosTS, BigPointValueTS, BigPointValueDWX, Precio, LotesCalculoRiesgo=0.01):
                """
                processor: objeto con acceso a processor.dwx.account_info (que contiene 'equity' y 'currency')
                CapitalTS: capital original en USD (asumimos que la estrategia TS lo da en USD)
                ContratosTS: número de contratos en la plataforma origen
                BigPointValueTS: valor del punto en la plataforma origen
                BigPointValueDWX: valor del punto en Darwinex
                Precio: valor EURUSD (1 EUR = Precio USD), se suscribe a este dato vía DarwinexConnect
                LotesCalculoRiesgo: lote base por defecto si no se puede calcular
                """
                ...
                # Ajustar CapitalTS según la divisa de la cuenta
                # Si la cuenta está en EUR y el capital se da en USD, convertir USD a EUR
                # 1 EUR = Precio USD => 1 USD = 1/Precio EUR => capital en EUR = CapitalTS(USD)/EURUSD
                if account_currency == "EUR":
                    ...
                    # El capital ya está en USD, no hacemos nada
                    pass
                else:
                    # Si la cuenta está en otra divisa distinta a EUR o USD, 
                    # deberías implementar una lógica adicional de conversión.
                    print("Cuenta en divisa desconocida, no se realiza conversión de capital.")
                    # CapitalTS se queda igual
                ...
                # Ajustar a un múltiplo del tamaño mínimo (0.01 lotes) y redondear siempre hacia abajo
                lot_step = 0.01
                # Usar floor para siempre ir hacia abajo
                lotes_rounded = math.floor(lotes_no_redondear / lot_step) * lot_step

                # Asegurar al menos el tamaño mínimo
            ...
            price = 0
            comment = workspace_
            lots = None  # Calcularemos el lote más tarde, dependiendo del processor

            # Para cada target (plataforma), calculamos los lotes y ejecutamos la lógica
            for processor in targets:
                ...
                #calculated_lots = calculaMM_DesdeCuentaTS(processor, capital_ts, quantity_, bpv_ts, bpv_dwx, 0.0)

                # Obtener si hay una orden abierta en este processor
                ...
                # Lógica de acuerdo al modo y acción
                if modo_ == "Largo":
                    if action_ == "Buy":
